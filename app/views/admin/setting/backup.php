...
<head>
    ...
    <title>مدیریت بک آپ ها | <?= $data['getPublicInfo']['site']; ?></title>
    <!-- Favicon -->
                            ...
                            <div class="table-responsive direction">
                                <table id="example1" class="table table-bordered table-striped" style="width: 100%">
                                    ...
                                    <tr>
                                        ...
                                        <th class="priority-1" style="text-align:center">تاریخ بک آپ</th>
        ...
        <div class="modal-dialog" style="width: 300px;">
            ...
            <div class="modal-content">
                <div class="modal-header" style="color: #fff;background: #2484c6;">
                    ...
                    <h4 class="modal-title">تغییر وضعیت استان</h4>
                </div>
                    ...
                    <div id="form-regular-delete" class="login-fold" style="display: inline;block">
                        <p class="email-wrap">
                            <label style="font-size: .9em;color: #777;display: inline-block;margin-top: 10px;font-weight: 700;">آیا
                                از تغییر وضعیت این استان اطمینان دارید؟</label>
                ...
                <div class="modal-footer"
                    ...
                    <span>در صورت فعال بودن، استان در لیست های مورد نیاز نمایش داده خواهد شد.</span>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body" style="padding: 0 15px;text-align: right;text-align: right;">
                    <div id="form-regular-delete" class="login-fold" style="display: inline;block">
                        <p class="email-wrap">
                            <label style="font-size: .9em;color: #777;display: inline-block;margin-top: 10px;font-weight: 700;">آیا
                                از حذف این استان اطمینان دارید؟</label>
                ...
                <div class="modal-footer"
                     style="margin-top: 10px !important;font-size: .8em;background: #f8f8f8;padding: 15px;text-align: right;border-bottom: 1px solid #e5e5e5;">
                    <span>توجه کنید در صورت حذف تمامی مطالب مربوط به این استان نیز حذف میگردد و امکان بازیابی نیز وجود ندارد.</span>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div id="form-regular-delete" class="login-fold row" style="display: inline;block">
                        <div class="col-md-12">
                            <div class="form-group" style="text-align:right">
                                <label style="width: 100%;" align="right" for="titleEdit">عنوان استان:</label>
    ...
    $(function () {
        ...
        var table = $('#example1').DataTable({
            ...
            "ajax": $.fn.dataTable.pipeline({
                url: "<?= ADMIN_PATH; ?>/getProvinceAjax",
                pages: 5
...
<script>
    ...
    $(document).on("click", "#status-submit", function () {
        ...
        if (navigator.onLine) {
            $.post(
                "<?= ADMIN_PATH; ?>/statusProvince",
                {
                    'id': id
                },
                function (data) {
                    if (data.status==="active") {
                        $.wnoty({type: 'success', message: 'استان مورد نظر باموفقیت فعال شد.'});
                        ...
                        $.wnoty({type: 'success', message: 'استان مورد نظر باموفقیت غیرفعال شد.'});
                        $("#btn-status-" + id).removeClass("btn-success").addClass("btn-danger");
                        document.getElementById("btn-status-" + id).innerHTML = 'غیرفعال';
                    } else if (data==="error") {
                        $.wnoty({type: 'warning', message: 'متاسفانه شما به این بخش دسترسی ندارید.'});
                    } else {
...
<script>
    $("#btnsubmit").on('click', function () {
        ...
        if (title == "") {
            $.wnoty({type: 'warning', message: 'عنوان استان را وارد کنید.'});
        } else {
            ...
            if (navigator.onLine) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.includes("ok")) {
                            $.wnoty({type: 'success', message: 'استان جدید با موفقیت ثبت شد.'});
                            location.reload();
                        } else if (data == "error") {
                            $.wnoty({type: 'warning', message: 'متاسفانه شما به این بخش دسترسی ندارید.'});
                        } else if (data.includes("exist")) {
                            $.wnoty({type: 'error', message: 'استان دیگری با این مشخصات قبلا ثبت شده است.'});
                        } else {
                            $.wnoty({type: 'error', message: 'متاسفانه خطایی رخ داده است.'});
                        }
                    }
    ...
    $(document).on("click", "#edit-submit", function () {
        ...
        if (navigator.onLine) {
            $.post(
                ...
                function (data) {
                    if (data == "ok") {
                        $.wnoty({type: 'success', message: 'استان مورد نظر باموفقیت ویرایش شد.'});
...
<script>
    ...
    $(document).on("click", "#delete-submit", function () {
        ...
        if (navigator.onLine) {
            $.post(
                "<?= ADMIN_PATH; ?>/delProvince",
                {
                    'id': id
                },
                function (data) {
                    if (data == "ok") {
                        $.wnoty({type: 'success', message: 'استان مورد نظر باموفقیت حذف شد.'});
