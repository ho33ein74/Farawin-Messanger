...
<head>
    ...
    <title>مدیریت رنگ ها | <?= $data['getPublicInfo']['site']; ?></title>
    <!-- Favicon -->
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                <small>لیست رنگ ها</small>
            </h1>
            <ol class="breadcrumb">
                <li><a href="<?= URL; ?><?= ADMIN_PATH; ?>/dashboard"><i class="fa fa-dashboard"></i> Home</a></li>
                <li><a href="<?= URL; ?><?= ADMIN_PATH; ?>/variants"><i class="fa fa-sliders"></i> Variants</a></li>
                <li><a href="<?= URL; ?><?= ADMIN_PATH; ?>/color"><i class="fa fa-flask"></i> Color</a></li>
            </ol>
        </section>
                ...
                <div class="col-md-4">
                    ...
                    <div class="box box-warning">
                        <div class="box-header with-border">
                            <h3 class="box-title">ثبت رنگ جدید</h3>
                                ...
                                <div class='row'>
                                    <div class='col-md-12'>
                                        <div class="form-group" style="text-align:right">
                                            <label style="width: 100%;" align="right" for="nameColor">:عنوان رنگ</label>
                                            <input style="border-radius: 3px;text-align:right" type="text"
                                                   class="form-control" id="nameColor" name="nameColor" required>
                                        </div>
                                    </div>
                                    <div class='col-md-12'>
                                        <div class="form-group" style="text-align:right">
                                            <label style="width: 100%;" align="right" for="codeColor">:کد رنگ (به همراه #)</label>
                                            <input style="border-radius: 3px;text-align:left;direction: ltr" type="text"
                                                   class="form-control my-colorpicker colorpicker-element" id="codeColor" name="codeColor" required>
                                        </div>
                                    </div>
                                </div>
                                <!-- /.box-body -->
                ...
                <div class="col-md-8">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">مدیریت رنگ ها</h3>
                        </div>
                        <!-- /.box-header -->
        ...
        <div class="modal-dialog" style="width: 300px;">
            ...
            <div class="modal-content">
                <div class="modal-header" style="color: #fff;background: #2484c6;">
                    ...
                    <h4 class="modal-title">حذف رنگ</h4>
                </div>
                    ...
                    <div id="form-regular-delete" class="login-fold" style="display: inline;block">
                        <p class="email-wrap">
                            <label style="font-size: .9em;color: #777;display: inline-block;margin-top: 10px;font-weight: 700;">آیا
                                از حذف این رنگ اطمینان دارید؟</label>
                ...
                <div class="modal-footer"
                    ...
                    <span>توجه کنید در صورت حذف تمامی  مطالب مربوط به این رنگ نیز حذف میگردد و امکان بازیابی نیز وجود ندارد.</span>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body" style="padding: 0 15px;text-align: right;text-align: right;">
                    <div id="form-regular-delete" class="login-fold row" style="display: inline;block">
                        <div class="col-md-12">
                            <div class="form-group" style="text-align:right">
                                <label style="width: 100%;" align="right" for="nameColorEdit">عنوان رنگ:</label>
                                <input style="border-radius: 3px;text-align:right" type="text" class="form-control" id="nameColorEdit" name="nameColorEdit" required="">
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group" style="text-align:right">
                                <label style="width: 100%;" align="right" for="codeColorEdit">کد رنگ:</label>
                                <input style="border-radius: 3px;text-align:left;direction: ltr" type="text" class="form-control my-colorpicker1" id="codeColorEdit" name="codeColorEdit" required="">
                            </div>
                        </div>
                        <p class="email-wrap">
...
<script type="text/javascript">
    $('.my-colorpicker, .my-colorpicker1').colorpicker();
</script>
    ...
    $(function () {
        ...
        var table = $('#example1').DataTable({
            ...
            "columnDefs": [
                ...
                {className: "priority-1", "targets": [0, 1, 2]},
                {className: "priority-2", "targets": []}
            ],
            "ajax": $.fn.dataTable.pipeline({
                url: '<?= ADMIN_PATH; ?>/getColorAjax',
                pages: 5
            })
        });
...
<script>
    $("#btnsubmit").on('click', function () {
        ...
        var code = document.getElementById("codeColor").value;
        if (name == "") {
            $.wnoty({type: 'warning', message: 'نام رنگ را وارد کنید.'});
        } else if (code == "") {
            $.wnoty({type: 'warning', message: 'کد رنگ را وارد کنید.'});
        } else {
            if (navigator.onLine) {
...
<script>
    $(document).on("click", "[id*=btn-edit-style-]", function () {
        document.getElementById("edit-val").value = $(this).data('id');
        $("#codeColorEdit").val($(this).data('code'));
        $("#nameColorEdit").val($(this).data('name'));
    });

    $(document).on("click", "#edit-submit", function () {
        $('#edit-Modal').modal('hide');
        var id = document.getElementById('edit-val').value;
        var code = document.getElementById('codeColorEdit').value;
        var name = document.getElementById('nameColorEdit').value;
        ...
        if (navigator.onLine) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.status == "ok") {
                        $("#btn-edit-style-"+id).parent().prev().html('<span style="border: 1px solid #000;width: 20px !important;height: 20px !important;display: inline-block;margin: 0px 0 0 5px !important;border-radius: 20px !important;vertical-align: middle;background-color: '+code+';"></span>'+name);

                        $("#bbtn-edit-style-"+id).data('code', code);
                        $("#btn-edit-style-"+id).data('name', name);
                    }
                },
            });
        } else {
