            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="box">
                        ...
                        <!-- /.box-header -->
                        <div data-step="1" data-intro="در این بخش لیست کامل دریافتی ها به شما نمایش داده می شود که به نکات زیر توجه نمایید:<br/><br/>1- برای گرفتن خروجی می توانید از دکمه های خروجی اکسل و یا خروجی csv بسته به نیاز استفاده نمایید.<br/><br/>2- برای پرینت اطلاعات می توانید از دکمه پرینت استفاده نمایید.<br/><br/>3- در صورتی که در هنگام گرفتن خروجی یا پرینت خواستید ستونی نمایش داده نشود می توانید از بخش فیلتر ستون ها، ستون مورد نظر خود را غیرفعال نمایید تا در خروجی مورد نظر نمایش داده نشود<br/><br/>4- به علت افزایش سرعت لود اطلاعات، اطلاعات به صورت محدود از دیتابیس خوانده می شود در صورتی که می خواهید لیست کامل اطلاعات را خروجی بگیرید کافیست از قسمت نمایش در پایین جدول گزینه همه را انتخاب و سپس اقدام به گرفتن خروجی نمایید.<br/><br/>5- برای جستجو و فیلتر کردن اطلاعات می توانید از فیلدهای موجود در زیر عنوان هر ستون استفاده نمایید و در صورت تمایل می توانید به صورت همزمان چند ستون را فیلتر نمایید تا اطلاعات مد نظر شما نمایش داده شود." class="box-body">
...
<script>
    $(function () {
        let part_state_inp, paymentType_state_inp = null;
        $('#example1 tfoot th').each(function () {
            var title = $(this).text();
            if (title == "ردیف") {
                $(this).html('-');
            } else if (title == "شماره سفارش") {
                $(this).html('-');
        ...
        var table = $('#example1').DataTable({
            ...
            "stateLoadParams": function(settings, data) {
                const  select_array = [0, "", 2, 3, 4, "", 6, 7, 8];
                        ...
                        if (select_array[i] !== "") {
                            ...
                            switch(i){
                                case 1:
                                    ...
                                    break;
                                case 5:
                                    paymentType_state_inp = col_search_val.replace("^", "").replace("$", "");
                                    break;
                            }
                        }
                    }
            ...
            "ajax": $.fn.dataTable.pipeline({
                ...
                this.api().columns(5).every(function () {
                    var column = this;
                    var select = $('<select><option value="0" disabled="" selected="" hidden="">انتخاب نوع پرداخت</option><option value="">همه موارد</option><option value="cash">صندوق (نقدی)</option><option value="bank">حساب بانکی</option></select>')
                        .appendTo($(column.footer()).empty())
                        .on('change', function () {
                            var val = $.fn.dataTable.util.escapeRegex(
                                $(this).val()
                    ...
                    $(select).children().each(function(d, j) {
                        if(j.value == paymentType_state_inp){
                            $(select).children().eq(d).attr("selected", true);
                        }
                    });
                });
