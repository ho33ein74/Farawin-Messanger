...
<head>
    ...
    <title>تنخواه گردان ها | <?= $data['getPublicInfo']['site']; ?></title>
    <!-- Favicon -->
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                <small>لیست تنخواه گردان ها</small>
            </h1>
            <ol class="breadcrumb">
                ...
                <div class="col-md-4">
                    ...
                    <div class="box box-warning">
                        <div class="box-header with-border">
                            <h3 class="box-title">افزودن تنخواه گردان جدید</h3>
                        </div>
                        <!-- /.box-header -->
                        <div data-step="1" data-intro="برای افزودن تنخواه جدید می بایست اطلاعات خواسته شده را تکمیل نمایید." class="box-body">
                            <div class="box-body">
                                <div class='row'>
                                    <div class='col-md-12'>
                ...
                <div class="col-md-8">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">مدیریت تنخواه گردان ها</h3>
                        </div>
                        <!-- /.box-header -->
                        <div data-step="3" data-intro="در این بخش لیست کامل تنخواه ها به شما نمایش داده می شود که به نکات زیر توجه نمایید:<br/><br/>1- برای گرفتن خروجی می توانید از دکمه های خروجی اکسل و یا خروجی csv بسته به نیاز استفاده نمایید.<br/><br/>2- برای پرینت اطلاعات می توانید از دکمه پرینت استفاده نمایید.<br/><br/>3- در صورتی که در هنگام گرفتن خروجی یا پرینت خواستید ستونی نمایش داده نشود می توانید از بخش فیلتر ستون ها، ستون مورد نظر خود را غیرفعال نمایید تا در خروجی مورد نظر نمایش داده نشود<br/><br/>4- به علت افزایش سرعت لود اطلاعات، اطلاعات به صورت محدود از دیتابیس خوانده می شود در صورتی که می خواهید لیست کامل اطلاعات را خروجی بگیرید کافیست از قسمت نمایش در پایین جدول گزینه همه را انتخاب و سپس اقدام به گرفتن خروجی نمایید.<br/><br/>5- برای جستجو و فیلتر کردن اطلاعات می توانید از فیلدهای موجود در زیر عنوان هر ستون استفاده نمایید و در صورت تمایل می توانید به صورت همزمان چند ستون را فیلتر نمایید تا اطلاعات مد نظر شما نمایش داده شود." class="box-body">
                            <div class="table-responsive direction">
                                <table id="example1" class="table table-bordered table-striped" style="width: 100%">
                                    <thead>
                                    <tr>
                                        <th class="priority-1" style="text-align:center;width: 30px">ردیف</th>
                                        <th class="priority-1" style="text-align:center">نام تنخواه گردان</th>
                                        <th class="priority-1" style="text-align:center">واحد پول</th>
        ...
        <div class="modal-dialog" style="width: 300px;">
            ...
            <div class="modal-content">
                <div class="modal-header" style="color: #fff;background: #2484c6;">
                    ...
                    <h4 class="modal-title">تغییر وضعیت تنخواه گردان</h4>
                </div>
                    ...
                    <div id="form-regular-delete" class="login-fold" style="display: inline;block">
                        <p class="email-wrap">
                            <label style="font-size: .9em;color: #777;display: inline-block;margin-top: 10px;font-weight: 700;">آیا
                                از تغییر وضعیت این تنخواه گردان اطمینان دارید؟</label>
                ...
                <div class="modal-footer"
                    ...
                    <span>در صورت فعال بودن، تنخواه گردان در لیست های مورد نیاز نمایش داده خواهد شد.</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div dir="rtl" class="modal fade" id="del-Modal" role="dialog">
        <div class="modal-dialog" style="width: 300px;">
            ...
            <div class="modal-content">
                <div class="modal-header" style="color: #fff;background: #2484c6;">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    <h4 class="modal-title">حذف تنخواه گردان</h4>
                </div>
                <div class="modal-body" style="padding: 0 15px;text-align: right;text-align: right;">
                    <div id="form-regular-delete" class="login-fold" style="display: inline;block">
                        <p class="email-wrap">
                            <label style="font-size: .9em;color: #777;display: inline-block;margin-top: 10px;font-weight: 700;">آیا
                                از حذف این تنخواه گردان اطمینان دارید؟</label>
                ...
                <div class="modal-footer"
                     style="margin-top: 10px !important;font-size: .8em;background: #f8f8f8;padding: 15px;text-align: right;border-bottom: 1px solid #e5e5e5;">
                    <span>توجه کنید در صورت حذف تمامی اطلاعات مربوط به این تنخواه گردان نیز حذف میگردد و امکان بازیابی نیز وجود ندارد.</span>
                </div>
            </div>
        </div>
    </div>
            ...
            if (navigator.onLine) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status == "ok") {
                            window.location.href = '<?= ADMIN_PATH; ?>/pettyCash';
                        }
                    }
                });
            } else {
...
<script>
    $(function () {
        ...
        <?php
        $currencyData = '<option value="">همه موارد</option>';
        foreach ($data['currency'] as $currency) {
            $currencyData .= '<option value="' . $currency['c_id'] . '">' . $currency['c_name'] . '-' . $currency['c_short_name'] . '</option>';
        }
        ?>
        ...
        var table = $('#example1').DataTable({
                ...
                for (i = 0; i < data.columns["length"]; i++) {
                    ...
                    if (col_search_val != "") {
                        if (select_array[i] !== "") {
                            ...
                            currency_state_inp = col_search_val.replace("^", "").replace("$", "");
                        }
                    }
                }
            },
            ...
            "ajax": $.fn.dataTable.pipeline({
                url: '<?= ADMIN_PATH; ?>/getpettyCashAjax',
                pages: 5
            }), initComplete: function () {
                this.api().columns(2).every(function () {
                    var column = this;
                    var select = $('<select><option value="0" disabled="" selected="" hidden="">انتخاب نوع هزینه</option><?= $currencyData; ?></select>')
...
<script>
    ...
    $(document).on("click", "#status-submit", function () {
        ...
        if (navigator.onLine) {
            ...
            $.ajax({
                url: "<?= ADMIN_PATH; ?>/statuspettyCash",
                data: formData,
                type: "POST",
                processData: false,
                contentType: false,
