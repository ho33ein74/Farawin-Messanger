...
<head>
    ...
    <title>هزینه‌های ثبت شده | <?= $data['getPublicInfo']['site']; ?></title>
    <!-- Favicon -->
                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">هزینه‌های ثبت شده</h3>
                        </div>
                        <!-- /.box-header -->
                        <div data-step="1" data-intro="در این بخش لیست کامل هزینه های ثبت شده به شما نمایش داده می شود که به نکات زیر توجه نمایید:<br/><br/>1- برای گرفتن خروجی می توانید از دکمه های خروجی اکسل و یا خروجی csv بسته به نیاز استفاده نمایید.<br/><br/>2- برای پرینت اطلاعات می توانید از دکمه پرینت استفاده نمایید.<br/><br/>3- در صورتی که در هنگام گرفتن خروجی یا پرینت خواستید ستونی نمایش داده نشود می توانید از بخش فیلتر ستون ها، ستون مورد نظر خود را غیرفعال نمایید تا در خروجی مورد نظر نمایش داده نشود<br/><br/>4- به علت افزایش سرعت لود اطلاعات، اطلاعات به صورت محدود از دیتابیس خوانده می شود در صورتی که می خواهید لیست کامل اطلاعات را خروجی بگیرید کافیست از قسمت نمایش در پایین جدول گزینه همه را انتخاب و سپس اقدام به گرفتن خروجی نمایید.<br/><br/>5- برای جستجو و فیلتر کردن اطلاعات می توانید از فیلدهای موجود در زیر عنوان هر ستون استفاده نمایید و در صورت تمایل می توانید به صورت همزمان چند ستون را فیلتر نمایید تا اطلاعات مد نظر شما نمایش داده شود." class="box-body">
                            <div class="table-responsive direction">
                                <table id="example1" class="table table-bordered table-striped" style="width: 100%">
                                    ...
                                    <tr>
                                        ...
                                        <th class="priority-1" style="text-align:center">نوع هزینه</th>
                                        <th class="priority-1" style="text-align:center;">مبلغ (ریال)</th>
                                        <th class="priority-2" style="text-align:center;">توضیحات</th>
        ...
        <div class="modal-dialog" style="width: 300px;">
            ...
            <div class="modal-content">
                <div class="modal-header" style="color: #fff;background: #2484c6;">
                    ...
                    <h4 class="modal-title">حذف هزینه</h4>
                </div>
                    ...
                    <div id="form-regular-delete" class="login-fold" style="display: inline;block">
                        <p class="email-wrap">
                            <label style="font-size: .9em;color: #777;display: inline-block;margin-top: 10px;font-weight: 700;">آیا
                                از حذف این هزینه اطمینان دارید؟</label>
                ...
                <div class="modal-footer"
                    ...
                    <span>توجه کنید در صورت حذف تمامی  مطالب مربوط به این هزینه نیز حذف میگردد و امکان بازیابی نیز وجود ندارد.</span>
                </div>
            </div>
        </div>
    </div>
...
<script>
    $(function () {
        let costType_state_inp, part_state_inp = null;
        ...
        <?php
        $typeData = '<option value="">همه موارد</option>';
        foreach ($data['costType'] as $type) {
            $typeData .= '<option value="' . $type['cost_category_vids_id'] . '">' . $type['title'] . '</option>';
        }
        ?>
        ...
        var table = $('#example1').DataTable({
                        ...
                        if (select_array[i] !== "") {
                            ...
                            switch(i){
                                case 1:
                                    ...
                                    break;
                                case 2:
                                    costType_state_inp = col_search_val.replace("^", "").replace("$", "");
                                    break;
                            }
                        }
                    }
            ...
            "ajax": $.fn.dataTable.pipeline({
                ...
                this.api().columns(2).every(function () {
                    var column = this;
                    var select = $('<select><option value="0" disabled="" selected="" hidden="">انتخاب نوع هزینه</option><?= $typeData; ?></select>')
                        .appendTo($(column.footer()).empty())
                        .on('change', function () {
                            var val = $.fn.dataTable.util.escapeRegex(
                                $(this).val()
