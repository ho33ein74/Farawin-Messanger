...
<head>
    ...
    <title>لیست کارهای انجام شده | <?= $data['getPublicInfo']['site']; ?></title>
    <!-- Favicon -->
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                <small>لیست کارهای انجام شده</small>
            </h1>
            <ol class="breadcrumb">
                ...
                <li class="active"><a><i class="fa fa-shopping-cart"></i> Orders</a></li>
            </ol>
        </section>
                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">لیست کارهای انجام شده</h3>
                        </div>
                        <!-- /.box-header -->
                        <div data-step="1" data-intro="در این بخش لیست کامل کارهای انجام شده پرسنل مورد نظر به شما نمایش داده می شود که به نکات زیر توجه نمایید:<br/><br/>1- برای گرفتن خروجی می توانید از دکمه های خروجی اکسل و یا خروجی csv بسته به نیاز استفاده نمایید.<br/><br/>2- برای پرینت اطلاعات می توانید از دکمه پرینت استفاده نمایید.<br/><br/>3- در صورتی که در هنگام گرفتن خروجی یا پرینت خواستید ستونی نمایش داده نشود می توانید از بخش فیلتر ستون ها، ستون مورد نظر خود را غیرفعال نمایید تا در خروجی مورد نظر نمایش داده نشود<br/><br/>4- به علت افزایش سرعت لود اطلاعات، اطلاعات به صورت محدود از دیتابیس خوانده می شود در صورتی که می خواهید لیست کامل اطلاعات را خروجی بگیرید کافیست از قسمت نمایش در پایین جدول گزینه همه را انتخاب و سپس اقدام به گرفتن خروجی نمایید.<br/><br/>5- برای جستجو و فیلتر کردن اطلاعات می توانید از فیلدهای موجود در زیر عنوان هر ستون استفاده نمایید و در صورت تمایل می توانید به صورت همزمان چند ستون را فیلتر نمایید تا اطلاعات مد نظر شما نمایش داده شود." class="box-body">
                            <div class="table-responsive direction">
                                <table id="example1" class="table table-bordered table-striped" style="width: 100%">
                                    ...
                                    <tr>
                                        ...
                                        <th class="priority-1" style="text-align:center;">تاریخ انجام</th>
                                        <th class="priority-1" style="text-align:center;width: 100px">مبلغ</th>
        ...
        <div class="modal-dialog" style="width: 300px;">
            ...
            <div class="modal-content">
                <div class="modal-header" style="color: #fff;background: #2484c6;">
                    ...
                    <h4 class="modal-title">تغییر وضعیت پرداخت</h4>
                </div>
                <div class="modal-body" style="padding: 0 15px;text-align: right;text-align: right;">
                    <div id="form-regular-delete" class="login-fold" style="display: inline;block">
                        <p class="email-wrap">
                            <label style="font-size: .9em;color: #777;display: inline-block;margin-top: 10px;font-weight: 700;">آیا از تغییر وضعیت این پرداخت اطمینان دارید؟</label>
                ...
                <div class="modal-footer" style="margin-top: 10px !important;font-size: .8em;background: #f8f8f8;padding: 15px;text-align: right;border-bottom: 1px solid #e5e5e5;">
                    <span>با تغییر وضعیت از حالت پرداخت نشده به حالت پرداخت شده تبدیل می شود.</span>
                </div>
            </div>
        </div>
    </div>
    ...
    $(function () {
        ...
        var table = $('#example1').DataTable({
                ...
                for (i = 0; i < data.columns["length"]; i++) {
                    ...
                    if (col_search_val != "") {
                        if (select_array[i] !== "") {
                            ...
                            saleStatus_state_inp = col_search_val.replace("^", "").replace("$", "");
                        }
                    }
                }
            },
            ...
            "ajax": $.fn.dataTable.pipeline({
                url: '<?= ADMIN_PATH; ?>/getOrderStaffAjax?id=<?= $data['attrId'] ?>',
                pages: 5
            }), initComplete: function () {
                this.api().columns(5).every(function () {
                    var column = this;
                    var select = $('<select><option value="0" disabled="" selected="" hidden="">انتخاب وضعیت</option><option value="" >همه موارد</option><option value="1" >پرداخت شده</option><option value="0" >پرداخت نشده</option></select>')
                    ...
                    $(select).children().each(function(d, j) {
                        if(j.value == saleStatus_state_inp){
...
<script>
    $(document).on("click", "[id*=btn-status-settlement-repair-]", function () {
    ...
    $(document).on("click", "#status-submit", function () {
        ...
        if (navigator.onLine) {
            ...
            $.ajax({
                url: "<?= ADMIN_PATH; ?>/statusSettlementSold",
                data: formData,
                type: "POST",
                processData: false,
                contentType: false,
                success: function (data) {
                    ...
                    if (data.status == "ok") {
                        $("#btn-status-settlement-repair-" + id).removeClass("btn btn-danger btn-xs").addClass("btn btn-success btn-xs");
                        document.getElementById("btn-status-settlement-repair-" + id).innerHTML = 'پرداخت شده';
                    }
                },
            });
        } else {
