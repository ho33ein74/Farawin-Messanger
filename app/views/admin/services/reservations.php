...
<head>
    ...
    <title>لیست نوبت های رزرو شده | <?= $data['getPublicInfo']['site']; ?></title>
    <!-- Favicon -->
...
<div class="wrapper">
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1>
                <small>لیست نوبت های رزرو شده</small>
            </h1>
            <ol class="breadcrumb">
                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header">
                            <h3 class="box-title">مدیریت نوبت های رزرو شده</h3>
                        </div>
                        <!-- /.box-header -->
                        <div data-step="1" data-intro="در این بخش لیست کامل فروش به شما نمایش داده می شود که به نکات زیر توجه نمایید:<br/><br/>1- برای گرفتن خروجی می توانید از دکمه های خروجی اکسل و یا خروجی csv بسته به نیاز استفاده نمایید.<br/><br/>2- برای پرینت اطلاعات می توانید از دکمه پرینت استفاده نمایید.<br/><br/>3- در صورتی که در هنگام گرفتن خروجی یا پرینت خواستید ستونی نمایش داده نشود می توانید از بخش فیلتر ستون ها، ستون مورد نظر خود را غیرفعال نمایید تا در خروجی مورد نظر نمایش داده نشود<br/><br/>4- به علت افزایش سرعت لود اطلاعات، اطلاعات به صورت محدود از دیتابیس خوانده می شود در صورتی که می خواهید لیست کامل اطلاعات را خروجی بگیرید کافیست از قسمت نمایش در پایین جدول گزینه همه را انتخاب و سپس اقدام به گرفتن خروجی نمایید.<br/><br/>5- برای جستجو و فیلتر کردن اطلاعات می توانید از فیلدهای موجود در زیر عنوان هر ستون استفاده نمایید و در صورت تمایل می توانید به صورت همزمان چند ستون را فیلتر نمایید تا اطلاعات مد نظر شما نمایش داده شود." class="box-body">
                            <div class="box-body">
                                <div class="table-responsive direction">
                                    <table id="example1" class="table table-bordered table-striped" style="width: 100%">
                                        <thead>
                                        <tr>
                                            <th class="priority-1" style="text-align:center;width: 20px">ردیف</th>
                                            <th class="priority-1" style="text-align:center">شماره پیگیری</th>
                                            <th class="priority-1" style="text-align:center">نام مشتری</th>
                                            <th class="priority-1" style="text-align:center">شعبه</th>
                                            <th class="priority-1" style="text-align:center">خدمت درخواستی</th>
                                            <th class="priority-1" style="text-align:center;width: 90px">زمان رزرو شده</th>
                                            <th class="priority-1" style="text-align:center;width: 90px">نوع رزرو</th>
                                            <th class="priority-1" style="text-align:center;">وضعیت</th>
                                            <th class="priority-1" style="text-align:center;width: 180px">عملیات</th>
                                        </tr>
                                        </thead>
                                        <tfoot>
                                        <tr>
        ...
        <div class="modal-dialog" style="width: 300px;">
            ...
            <div class="modal-content">
                <div class="modal-header" style="color: #fff;background: #2484c6;">
                    ...
                    <h4 class="modal-title">حذف نوبت</h4>
                </div>
                    ...
                    <div id="form-regular-delete" class="login-fold" style="display: inline;block">
                        <p class="email-wrap">
                            <label style="font-size: .9em;color: #777;display: inline-block;margin-top: 10px;font-weight: 700;">آیا
                                از حذف این نوبت اطمینان دارید؟</label>
                ...
                <div class="modal-footer" style="margin-top: 10px !important;font-size: .8em;background: #f8f8f8;padding: 15px;text-align: right;border-bottom: 1px solid #e5e5e5;">
                    <span>توجه کنید در صورت حذف تمامی اطلاعات این نوبت رزرو شده نیز حذف میگردد و امکان بازیابی نیز وجود ندارد.</span>
                </div>
            </div>
        </div>
    </div>
...
<script>
    $(function () {
        let branches_state_inp, type_state_inp, status_state_inp, service_state_inp = null;
        $('#example1 tfoot th').each(function (order_col) {
            var title = $(this).text();
            if (title == "ردیف") {
                $(this).html('-');
                ...
                let date = "<?= $data['date_search'] ?>";
                if(order_col == 5 && date!="") {
                    date = date.replace(/-/g, "/");
                    $(this).html('<input disabled class="setInputValue" style="text-align: start;unicode-bidi: plaintext;" value="' + date + '" type="text" placeholder="جستجو ' + title + '" />');
                } else {
        ...
        <?php
        $statusData = '<option value="">همه موارد</option>';
        foreach ($data['status'] as $status) {
            $statusData .= '<option value="' . $status['title'] . '">' . $status['title'] . '</option>';
        }

        $serviceData = '<option value="">همه موارد</option>';
        foreach ($data['services'] as $service) {
            $serviceData .= '<option value="' . $service['s_title'] . '">' . $service['s_title'] . '</option>';
        }

        $branchesData = '<option value="">همه موارد</option>';
        foreach ($data['getBranches'] as $branch) {
            $branchesData .= '<option value="' . $branch['branch_vids_id'] . '">' . $branch['b_name'] . '</option>';
        }
        ?>
        ...
        var table = $('#example1').DataTable({
                ...
                for (i = 0; i < data.columns["length"]; i++) {
                    ...
                    if (col_search_val != "") {
                        if (select_array[i] !== "") {
                            ...
                            switch(i){
                                case 3:
                                    branches_state_inp = col_search_val.replace("^", "").replace("$", "");
                                    break;
                                case 4:
                                    service_state_inp = col_search_val.replace("^", "").replace("$", "");
                                    break;
                                case 6:
            ...
            "ajax": $.fn.dataTable.pipeline({
                url: '<?= ADMIN_PATH; ?>/getReservationsListAjax',
                data: { date: $(".setInputValue").val() },
                pages: 5
            }), initComplete: function () {
                this.api().columns(3).every(function () {
                    var column = this;
                    var select = $('<select><option value="0" disabled="" selected="" hidden="">انتخاب شعبه</option><?= $branchesData; ?></select>')
                ...
                this.api().columns(4).every(function () {
                    var column = this;
                    var select = $('<select><option value="0" disabled="" selected="" hidden="">انتخاب خدمت</option><?= $serviceData; ?></select>')
                        .appendTo($(column.footer()).empty())
                        .on('change', function () {
                            var val = $.fn.dataTable.util.escapeRegex(
                                $(this).val()
                ...
                this.api().columns(6).every(function () {
                    var column = this;
                    var select = $('<select><option value="0" disabled="" selected="" hidden="">انتخاب نوع</option><option value="">همه موارد</option><option value="1">ویژه</option><option value="0">عادی</option></select>')
                        .appendTo($(column.footer()).empty())
                        .on('change', function () {
                            var val = $.fn.dataTable.util.escapeRegex(
                                $(this).val()
                ...
                this.api().columns(7).every(function () {
                    var column = this;
                    var select = $('<select><option value="0" disabled="" selected="" hidden="">انتخاب وضعیت</option><?= $statusData; ?></select>')
                        .appendTo($(column.footer()).empty())
                        .on('change', function () {
                            var val = $.fn.dataTable.util.escapeRegex(
                                $(this).val()
