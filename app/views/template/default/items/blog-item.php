<div class="shadow-sm p-4 flex flex-col dark:bg-dark-930 bg-white rounded h-full">

    <a class="rounded flex w-full md:h-40 sm:h-60 h-44 overflow-hidden" href="blog/article/<?= $news['slug'] ?>">
        <img data-src="public/images/blog/<?= $news['cover'] ?>" src="public/images/blog/<?= $news['cover'] ?>" onerror="this.src='public/images/default_cover.jpg'"
             class="lozad w-full h-full object-cover transform transition duration-200 hover:scale-110" alt="<?= $news['title'] ?>" />
    </a>
        ...
        <div class="flex flex-col flex-grow">
            <h4 class="mt-3 mb-4 ">
                <a class="font-bold text-xl text-justify transition duration-200 dark:hover:text-blue-450 hover:text-blue-700 dark:text-white text-gray-800 h-16 overflow-hidden"
                   href="blog/article/<?= $news['slug'] ?>"><?= $news['title'] ?></a>
            </h4>
        </div>
        ...
        <div>
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center justify-between">
                    <img class="rounded-full w-6 h-6 object-cover ml-1" src="<?= URL ?><?= $news['writerImage'] ?>" alt="<?= $news['writer'] ?>" onerror="this.src='public/images/user-default-image.jpg'"/>
                    <h6>
                        <a class="dark:hover:text-blue-450 dark:text-white text-gray-400 font-semibold text-xs transition duration-200 hover:text-gray-800" href="blog?author=<?= $news['writerId']; ?>">
                            <?= $news['writer'] ?>
                        </a>
                    </h6>
                </div>
                ...
                <div class="flex items-center space-x-reverse space-x-2">
                    <a href="blog/category/<?= $news['link']; ?>" class="group flex dark:hover:!text-white dark:bg-blue- dark:hover:text-white dark:!bg-opacity-10 dark:text-blue-950 text-blue-700 items-center px-2 font-semibold text-xs  py-1 rounded bg-blue-40 transition duration-200 hover:bg-blue-700 hover:text-white">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    <a href="blog/article/<?= $news['slug'] ?>#comments" class="flex items-center px-1 h-5 ml-2 text-xs text-gray-500 dark:hover:bg-gray-940 dark:hover:text-gray-500 dark:bg-gray-500 dark:bg-opacity-10 dark:text-gray-940 bg-gray-500 bg-opacity-10 rounded">
                    ...
                    <button data-id="<?= $news['n_id']; ?>" data-type="blog" data-part="blog" data-view="text" class="flex group add-like <?= isset($data['userId']) and $data['userId']!=FALSE ? "":" login_req";  ?> items-center px-1 h-5 ml-2 text-xs text-red-450 bg-red-700 dark:text-red-650 dark:hover:text-white dark:hover:bg-red-650 bg-opacity-10 rounded transition duration-200 hover:bg-opacity-100 hover:text-white">
                        <svg id="likeIcon-<?= $news['n_id']; ?>" class="ml-1 -mt-0.5" fill="<?= $news['liked']!=NULL ? "currentColor":" none";  ?>" width="13" height="11" viewBox="0 0 13 11" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <span class="likeCounter-<?= $news['n_id']; ?>"><?= $news['likeCount'] ?></span>
                    </button>

                    <button data-id="<?= $news['n_id']; ?>" data-type="blog" class="group flex items-center px-1 dark:hover:bg-blue-970 dark:hover:text-blue-700 dark:bg-blue-700 dark:text-blue-970 dark:bg-opacity-10 bg-blue-700 bg-opacity-10 h-5 rounded transition duration-200 hover:bg-opacity-100 add-bookmark <?= $news['bookmarked']!=NULL ? "text-blue-700":"text-gray-300";  ?> <?= isset($data['userId']) and $data['userId']!=FALSE ? "":" login_req";  ?>">
                        <svg id="bookmarkIcon-<?= $news['n_id']; ?>" width="10" height="12" fill="<?= $news['bookmarked']!=NULL ? "currentColor":"none";  ?>" viewBox="0 0 10 12" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="flex items-center font-medium text-xs dark:text-gray-980 text-gray-400">
                    ...
                    زمان مطالعه: <?= Model::get_read_time(htmlspecialchars_decode($news['description'])) ?> دقیقه
                </div>
            </div>
        </div>
    </div>
