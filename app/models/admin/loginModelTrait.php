...
trait loginModelTrait
{
    function Login($form)
    {
        try {
            ...
            $password = $this->Check_Param($this->hash_value_sha1($this->hash_value_md5($form['password'])));

            if (!empty($username) and !empty($password)) {
                $result = $this->doSelect("SELECT * FROM tbl_admin WHERE a_username=? AND a_password=? AND a_status=1", array($username, $password), 1);

                if (sizeof($result) > 0 and $result != "") {
                    if ($result['google_auth_status'] == 0) {
                        $this->cookieInit();
                        $this->cookieSet('adminId', $this->Encrypt($result['a_id'], KEY), $this->getPublicInfo('cookie_duration'));
                        $this->ActivityLog("ورود به پنل مدیریت", $result['a_id']);
                        $this->response_success(".باموفقیت وارد شدید :)");
                    } else {
                        $this->response_success("برای ورود می بایست کد تایید گوگل را وارد نمایید", "auth");
                    }
                } else {
                    $this->response_error(".اطلاعات وارد شده صحیح نمی باشد<br/>لطفا مجددا تلاش نمایید");
                }
            } else {
    ...
    function authCheck($form)
    {
        try {
            ...
            if (sizeof($result) > 0 and !empty($username)) {
                ...
                if ($pga->verifyCode($result['google_secret_code'], $form['code'], 2)) {
                    $this->cookieInit();
                    $this->cookieSet('adminId', $this->Encrypt($result['a_id'], KEY), $this->getPublicInfo('cookie_duration'));
                    $this->ActivityLog("ورود به پنل مدیریت", $result['a_id']);
    ...
    function forgetPasswordSendSMS($form)
    {
        try {
            ...
            if (sizeof($result) > 0 and !empty($username)) {
                ...
                $password = $this->Check_Param($this->hash_value_sha1($this->hash_value_md5($code)));
