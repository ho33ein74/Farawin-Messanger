    ...
    function calculateTimeDifferent(endTime, type, timer_id, wrapper_id, timesup_wrapper_id, local_s_days, local_s_hrs, local_s_min, local_s_sec, local_l_days, local_l_hrs, local_l_min, local_l_sec) {
        ...
        if (difference <= 0) {
            ...
            if ("text" === type) {
                ...
                if (10 > min) { min = "0" + min; }
                if (10 > sec) { sec = "0" + sec; }
                var str = '<div class="countdown_box bg-blue-700 border-transparent img-grayscale-0 text-white dark:text-black dark:bg-gray-900 dark:border-opacity-0 countdown_day"><b>' + days + '</b><br />' + local_l_days + '</div>';
                str += '<div class="countdown_box bg-blue-700 border-transparent img-grayscale-0 text-white dark:text-black dark:bg-gray-900 dark:border-opacity-0 countdown_hours"><b>' + hrs + '</b><br />' + local_l_hrs + '</div>';
                str += '<div class="countdown_box bg-blue-700 border-transparent img-grayscale-0 text-white dark:text-black dark:bg-gray-900 dark:border-opacity-0 countdown_mins"><b>' + min + '</b><br />' + local_l_min + '</div>';
                str += '<div class="countdown_box bg-blue-700 border-transparent img-grayscale-0 text-white dark:text-black dark:bg-gray-900 dark:border-opacity-0 countdown_sec"><b>' + sec + '</b><br />' + local_l_sec + '</div>';
                $(timer_id).html(str);
            }
            setTimeout(function () {
